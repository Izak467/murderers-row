...
- There is no pre-game mode toggle — every game is played the same way. Sender plays a normal solo game; the **results screen shows the share/challenge row (Challenge a Friend, Play Again, Share Score) right under the score, above the lineup card**, so it's the first thing visible. Tapping "Challenge a Friend" turns that into a send card ("Your Move Is In" → "⏳ Their Move" once sent). The Firebase doc is **pre-created when the card renders** (`_h2hPrepare`) so `navigator.share` fires synchronously on tap (iOS drops the share sheet if the user gesture expires during an async save).
